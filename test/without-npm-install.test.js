...
/**
 * TODO: Finalize non-npm installed behavior.
 *
 * - Resolved package versions depends on `npm install` having been run.
 */
...
test('without-npm-install', { skip: true }, function cb (t) {
  var dir = path.resolve(fixtures, 'without-npm-install')
  ...
  // console.log(expectedJson)
    ...
    t.equal(
      data.devDependencies && Object.keys(data.devDependencies).length
      , 1
      , 'devDependency entries'
    )
